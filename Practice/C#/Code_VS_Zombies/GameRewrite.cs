...
class Zombie
{
    ...
    public void SetClosestHuman(Human human, int nbTurnToReach)
    {
        if (human == null)
        {
            ClosestHumanId = -1;
            Target = null;
            NbTurnToReachHuman = nbTurnToReach;
        }
        else
        {
...
static class SimulationGame
{
    ...
    public static void FindZombieTarget(Zombie zombie, SimulationInfos simulationInfos)
    {
        ...
        {
            tmpDist = GetDistance(zombie.Position, human.Position);
            if (tmpDist <= minDist)
            {
    ...
    public static bool NextPosZombie(Zombie zombie, PlayerNash nash, out Tuple<int, int> posOut)
    {
        ...
        dist = GetDistance(zombie.Position, targetPos);
        ...
        else
        {
            ...
            posOut = new Tuple<int, int>(
                zombie.Position.Item1 + (int)Math.Floor(t * (targetPos.Item1 - zombie.Position.Item1)),
                zombie.Position.Item2 + (int)Math.Floor(t * (targetPos.Item2 - zombie.Position.Item2))
                );
        }
    ...
    public static bool NextPosNash(PlayerNash nash, out Tuple<int, int> posOut)
    {
        ...
        if (nash.Target != null || nash.NextPosition != null)
        {
            ...
            distance = GetDistance(nash.Position, destination);
            ...
            else
            {
                ...
                posOut = new Tuple<int, int>(
                        nash.Position.Item1 + (int)Math.Floor(t * (destination.Item1 - nash.Position.Item1)),
                        nash.Position.Item2 + (int)Math.Floor(t * (destination.Item2 - nash.Position.Item2))
                    );
            }
        }
        else
